...
- The bundled hands currently shipped are `scheduled-maintenance`,
  ...
  `provider-health-watchdog`, and `docs-drift`.
- **Autonomous Engines (Phases 12-15)**: `economos`, `sophia`, `architectura`, and `aeturnus` join the factory's core loop, providing proactive optimization and self-healing.
- Each is visible in operator controls and starts paused until an operator activates it.
...
- The shipped self-improvement foundation has reached **Autonomous-Ready (Phase 15)**: `improvement-hand`, `user-correction-miner`, and `regression-watch` write structured proposals; the factory can now autonomously promote high-confidence improvements when Governance Gates are met.
