...
async function executeBundledHandRun(input: {
  ...
  try {
    ...
    await repository.appendToolEvent({
      timestamp: input.timestamp,
      toolName: HAND_RUN_TOOL,
      summary: `${input.definition.label} reviewed ${regressionReview.reviewedSignalCount} stored signal(s), recorded ${regressionReview.findingCount} bounded finding(s), and wrote ${regressionReview.generatedProposalCount} structured proposal(s) for cron ${input.cron}.`,
      metadata: {
        action: "run",
        cron: input.cron,
